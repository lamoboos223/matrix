...
class BroadcastCommand(Command):
    ...
    def __init__(self, request_id: str, page: int, topic_ids: str,
                 access_token_id: int):
    ...
    @classmethod
    def from_line(
        cls: Type["BroadcastCommand"], line: str
    ) -> "BroadcastCommand":
        request_id, page, topic_ids, access_token_id = line.split(" ")
        return cls(request_id, page, topic_ids, access_token_id)

    def to_line(self) -> str:
        return "%s %s %s %s" % (self.request_id, self.page, self.topic_ids,
