...
class WorkerConfig(Config):
    ...
    def _append_worker_to_writers_and_instance_map(self, config):
        if self.instance_name not in self.instance_map:
            self.instance_map[self.instance_name] = InstanceLocationConfig(
                host=config.get("instance").get("host"),
                port=config.get("instance").get("port"))
