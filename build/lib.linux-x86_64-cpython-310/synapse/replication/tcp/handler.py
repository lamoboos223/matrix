...
class ReplicationCommandHandler:
    ...
    # TWK
    def on_BROADCAST(
        self, conn: IReplicationConnection, cmd: BroadcastCommand
    ) -> Optional[Awaitable[None]]:
        if self._is_master:
            return
    ...
    async def emit_message(self, sender, event_dict, request_id):
        ...
        try:
            ...
            event, _ = await self.event_creation_handler. \
                create_and_send_nonmember_event(
                    sender, event_dict, txn_id=request_id
                )
            if event is None:
    ...
    async def _handle_broadcast(self, cmd: BroadcastCommand, retry_count=3) -> None:
        """
        BroadcastCommand is sent as per page so only room_ids
        of the specified page cmd.page are considered.

        If total # of users is 300 and the configured page size is 50 then
        data is to be collected 6 times (6 pages) and
        BroadcastCommand is to be sent 6 times
        """
        if retry_count <= 0:
            return

        room_ids = await self._store.get_room_ids_by_request_id(
            cmd.request_id,
            cmd.page,
            len(self.events_shard_config.instances),
            self.events_shard_config.instances.index(self._instance_name))

        request_data = await self._store.get_request_data(cmd.request_id)
        message_content = {
            "msgtype": "m.text",
